...
pub const OpenAiCompatibleProvider = struct {
    ...
    fn collectResponsesInstructions(allocator: std.mem.Allocator, messages: []const ChatMessage) !?[]u8 {
        ...
        for (messages) |msg| {
            ...
            try appendResponsesMessageText(&buf, allocator, msg);
            if (buf.items.len == before + 2) {
                buf.items.len = before;
            }
        }
    ...
    fn appendResponsesContentPart(
        ...
        switch (part) {
            ...
            .image_url => |img| {
                try buf.appendSlice(allocator, "{\"type\":\"input_image\",\"source\":{\"type\":\"url\",\"url\":");
                try root.appendJsonString(buf, allocator, img.url);
                try buf.appendSlice(allocator, "}}");
            },
            .image_base64 => |img| {
                try buf.appendSlice(allocator, "{\"type\":\"input_image\",\"source\":{\"type\":\"base64\",\"media_type\":");
    ...
    fn serializeResponsesInputInto(
        ...
        for (messages) |msg| {
            ...
            switch (msg.role) {
                ...
                .tool => {
                    try buf.appendSlice(allocator, "{\"type\":\"function_call_output\",");
                ...
                for (output_arr.array.items) |item| {
                    ...
                    if (item_obj.get("content")) |content_arr| {
                        ...
                        for (content_arr.array.items) |content| {
                            ...
                            const text_val = cobj.get("text") orelse continue;
    ...
    fn extractResponsesReasoning(
        ...
        for (output_arr.array.items) |item| {
            ...
            if (item_obj.get("summary")) |summary| {
                if (summary == .string and summary.string.len > 0) return try allocator.dupe(u8, summary.string);
            }
    ...
    pub fn parseResponsesResponse(allocator: std.mem.Allocator, body: []const u8) !ChatResponse {
        ...
        const model_str = if (root_obj.get("model")) |m|
            if (m == .string) try allocator.dupe(u8, m.string) else try allocator.dupe(u8, "")
        else
            try allocator.dupe(u8, "");
    ...
    fn streamChatImpl(
        ...
        if (self.api_mode == .responses) {
            const fallback = try chatImpl(ptr, allocator, request, effective_model, temperature);
...
test "buildResponsesRequestBody includes image parts" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, body, "\"data\":\"Zm9v\"") != null);
}

test "parseResponsesResponse extracts function call and text" {
    const body =
        \\{"model":"gpt-5.4","usage":{"input_tokens":10,"output_tokens":5,"total_tokens":15},"output":[{"type":"message","role":"assistant","content":[{"type":"output_text","text":"Need tool"}]},{"type":"function_call","call_id":"call_1","name":"bash","arguments":"{\"command\":\"ls\"}"}]}
    ;
